...
int main() {
    ...
    for (double x = x_min; x <= x_max; x += dx) {
        ...
        data1.push_back(std::make_pair(x, y1));
        data2.push_back(std::make_pair(x, y2));
        data3.push_back(std::make_pair(x, y3));
    }
    ...
    gp.send(data1);  // Envoie des données à gnuplot pour la première courbe
    gp.send(data2);  // Envoie des données à gnuplot pour la deuxième courbe
    gp.send(data3);  // Envoie des données à gnuplot pour la troisième courbe
